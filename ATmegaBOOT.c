...
/* DW.CLOUD v2.0 Only */
#ifdef DW_CLOUD_V20
#define ESP_BTN_DDR     DDRC
#define ESP_BTN_PORT    PORTC
#define ESP_BTN_PIN     PINC
#define ESP_BTN         (1 << 4)

#define ESP_RST_DDR     DDRD
#define ESP_RST_PORT    PORTD
#define ESP_RST_PIN     PIND
#define ESP_RST         (1 << 7)

#define ESP_BOOT_DDR    DDRC
#define ESP_BOOT_PORT   PORTC
#define ESP_BOOT_PIN    PINC
#define ESP_BOOT        (1 << 3)

#define ESP_SPEED       115200
#endif
...
#elif defined(__AVR_ATmega324P__) || defined(__AVR_ATmega324PA__)
...
#ifdef DW_CLOUD_V20
    ESP_BTN_DDR &= ~ESP_BTN;
    ESP_BTN_PORT |= ESP_BTN;

    ESP_RST_DDR |= ESP_RST;
    ESP_RST_PORT &= ~ESP_RST;

    ESP_BOOT_DDR |= ESP_BOOT;
    ESP_BOOT_PORT &= ~ESP_BOOT;

    /* Check for ESP_BTN was pressed */
    if ((ESP_BTN_PIN & ESP_BTN) == 0)
    {
        /* Setting USART0 and USART1 to ESP8266 BOOT SPEED with UART double speed mode*/
        UCSR0A |= (1 << U2X0);
        UBRR0 = (uint16_t)(F_CPU/(ESP_SPEED*8L)-1);
        UCSR0B = (1<<RXEN0) | (1<<TXEN0);
        UCSR0C = (1<<UCSZ00) | (1<<UCSZ01);

        UCSR1A |= (1 << U2X1);
        UBRR1 = (uint16_t)(F_CPU/(ESP_SPEED*8L)-1);
        UCSR1B = (1<<RXEN1) | (1<<TXEN1);
        UCSR1C = (1<<UCSZ10) | (1<<UCSZ11);

        /* Enable ESP8266 boot mode */
        ESP_BOOT_PORT |= ESP_BOOT;
        ...
        /* Release ESP8266 from reset */
        ESP_RST_PORT &= ~ESP_RST;

        /* Uart forwarding loop */
        while (1) {
